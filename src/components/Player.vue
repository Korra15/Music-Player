    ...
    <div class="relative">
      ...
      <div class="float-left w-7 h-7 leading-3">
        <button type="button" @click.prevent="toggleAudio">
          <i class="fa text-gray-500 text-xl"
            ...
            <!-- binding the attribute as we want the classed to be dyanmic
                 they can be added using object syntax -->
        </button>
      </div>
      <!-- Current Position -->
      ...
      <div class="float-left w-7 h-7 leading-3 ml-7 mt-2 player-scrub">
        <div class="absolute left-0 right-0 text-lg text-center mx-auto player-song-info"
          ...
          <span class="song-title">{{ currentSong.modifiedName }}</span> ( Uploaded by
        ...
        <span class="block w-full h-2 rounded m-1 mt-2 bg-gray-200 relative cursor-pointer"
         @click.prevent="updateSeek">
          ...
          <span class="absolute top-neg-8 text-gray-800 text-lg"
            ...
            <!-- binding the style attribute with the object syntax -->
...
export default {
  name: 'Player',
  computed: {
    // getters are equivalent to the computed property for the state
    ...mapGetters(['playing']),
